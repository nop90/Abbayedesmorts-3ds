...
#ifdef _3DS		
			if (evento.key.keysym.sym == KEY_JUMP || evento.key.keysym.sym == KEY_ACTION) {
#else
...
#ifdef _3DS
			if (evento.key.keysym.sym == SDLK_y)
#else			
   	...
   	if (evento.type == SDL_KEYDOWN) {
	   	...
	   	if (evento.key.keysym.sym == SDLK_ESCAPE)
			...
			if (evento.key.keysym.sym == KEY_GRAPHICS)
				*tecla = 9;
#ifdef _3DS
			if (evento.key.keysym.sym == KEY__START || evento.key.keysym.sym == KEY_ACTION)
#else
			if (evento.key.keysym.sym == KEY__TART)
#endif
				*tecla = 5;
		}
	}
